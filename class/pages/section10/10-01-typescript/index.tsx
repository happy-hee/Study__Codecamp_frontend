export default function TypescriptPage() {
  ...
  // 타입 명시
  const bbb: string = "반갑습니다";
  // bbb = 10; // error

  // 타입 명시가 필요한 상황
  // nunber도 되고 string도 되는 타입이라고 명시를 해야함.
  const ccc: number | string = 1000;
  // ccc = "1000원";
  ...
  // boolean타입
  const eee: boolean = true;
  // eee = false;
  // eee = "false"; // error, 또한 비어있지 않은 문자열은 true 이므로 false 라고 되어있어도 true가 되니 주의가 필요

  // 배열타입
  // const fff: number[] = [1, 2, 3, 4, 5, "안녕하세요"]; // 숫자들만 들어갈 수 있는 배열. "안녕하세요" 부분이 error
  // const ggg: string[] = ["반", "갑", "습", "니", "다", 10]; // 문자열만 들어갈 수 있는 배열. 10 부분이 error
  const hhh = ["반", "갑", "습", "니", "다", 10]; // 타입을 추론해서 어떤 타입을 사용하는지 알아보기 (변수명에 마우스를 가져다대면 추론된 타입이 나옴.)
  ...
  console.log(aaa, bbb, ccc, ddd, eee, hhh, result, result2, qqq)
